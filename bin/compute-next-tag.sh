...
#
# Tags look like `v<Changedetection.io version>-<release>`:
#
# - if defaults/main.yml points at a Changedetection.io version that has never
#   been released, the release counter restarts at 0 (`v0.55.8-0`)
# - otherwise the counter is incremented (`v0.55.8-1`), but only if something
...
#
# The role also deploys a Playwright driver container, but its version is
# pinned to `latest` and Renovate never bumps it, so there is no second version
# to key tags on. A change that does affect that container is a change to the
# role's own files, which rolls the release counter like any other.
...
version="$(sed -nE 's|^changedetection_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Changedetection.io version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `0.55.8`), but the tags
# do (`v0.55.8-0`). Stripping any `v` before prepending one keeps this correct
